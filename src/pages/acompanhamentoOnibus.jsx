...
function AcompanhamentoOnibus() {
  ...
  const buscaCoordenadasParadas = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_BUS_API}/coordenadas-intinerario?codItinerario=${codItinerario}`)

      setCoordenadasParadas(data.map(c => ({ lat: c.coordY, lng: c.coordX })))
    } catch (error) {
      console.log(error)
    }
  }
